...
public class mysqlConnection {
        ...
        try (PreparedStatement ps = conn.prepareStatement(query)) {
            ...
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
                    ...
                    activityHistory.add("Book Name: " + bookName + ", Action: " + actionType + 
    ...
    public static ArrayList<String> BringBorrowRepInfo() throws SQLException {
        ...
        try (PreparedStatement ps = conn.prepareStatement(query1)) {
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
                    FullBorrowRep.add(String.format("Subscriber ID: %s    Book Name: %s    Borrow Date: %s    Return Date: %s    Additional Details: %s",
        ...
        try (PreparedStatement ps = conn.prepareStatement(query2)) {
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
                    FullBorrowRep.add(String.format("Subscriber ID: %s    Book Name: %s    Borrow Date: %s    Return Date: __-__-____ __:__:__     Additional Details: N/A",
                        rs.getString("SubscriberID"),
                        rs.getString("BookName"),
                        rs.getString("BorrowDate")));
                }
